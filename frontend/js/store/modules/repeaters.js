...
const getters = {
  repeatersByBlockId: state => id => {
    const ids = Object.keys(state.repeaters).filter(key =>
...
const actions = {
  async [ACTIONS.DUPLICATE_REPEATER](
    { state, commit, getters },
    { editorName, block, index, id }
  ) {
  ...
  async [ACTIONS.DUPLICATE_BLOCK]({ commit, getters }, { block, id }) {
    ...
    repeaterIds.forEach(
      repeaterId =>
        (duplicates[repeaterId.replace(block.id, id)] = [
