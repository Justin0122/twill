  ...
  export default {
    ...
    computed: {
      switcherClasses: function() {
        return [
          ...
          this.formatTextEnabled
            ? `switcher--${lowercase(this.formatTextEnabled)}`
            : ''
        ]
      },
      isChecked: function() {
      ...
      formatTextEnabled: function() {
        const scoreStart = compareAsc(
          this.startDate ? parseISO(this.startDate) : null,
          new Date()
        )
        const scoreEnd = compareAsc(
          this.endDate ? parseISO(this.endDate) : null,
          new Date()
        )
