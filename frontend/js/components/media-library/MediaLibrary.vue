...
<script>
  ...
  const FolderNode = {
    ...
    methods: {
      ...
      selectSelf () { this.$emit('select', this.pathHere()) },
      createHere () { this.$emit('create', this.pathHere()) }
    },
    template: `
  ...
  export default {
    ...
    props: {
      modalTitlePrefix: {
        type: String,
        default: function () { return this.$trans('media-library.title', 'Media Library') }
      },
      btnLabelSingle: {
        type: String,
        default: function () { return this.$trans('media-library.insert', 'Insert') }
      },
      btnLabelUpdate: {
        type: String,
        default: function () { return this.$trans('media-library.update', 'Update') }
      },
      btnLabelMulti: {
        type: String,
        default: function () { return this.$trans('media-library.insert', 'Insert') }
      ...
      extraMetadatas: { type: Array, default () { return [] } },
      translatableMetadatas: { type: Array, default () { return [] } }
    },
    data () {
      return {
        loading: false,
    ...
    computed: {
      ...
      endpoint () { return this.currentTypeObject.endpoint },
      modalTitle () {
        if (this.connector) {
      ...
      usedMedias () { return this.selected[this.connector] || [] },
      selectedType () {
        const navItem = this.types.filter(t => t.value === this.type)
        return navItem[0]
      },
      canInsert () {
    ...
    watch: {
      type () {
        // When switching types, restore last path for that type and reload tree/grid
    ...
    methods: {
      /* ---------- persistence helpers ---------- */
      storageKey () {
        // namespace by endpoint + type to avoid collisions between media/file libraries
        return `twill:ml:lastFolder:${this.endpoint}:${this.type}`
      },
      saveLastFolder () {
        const key = this.storageKey()
        const value = this.currentFolderFullPath // string like "foo/bar" or ""
        try {
          window.localStorage.setItem(key, value)
        } catch (e) {
          // cookie fallback ~1 year
          document.cookie = encodeURIComponent(key) + '=' + encodeURIComponent(value) + '; max-age=' + (60 * 60 * 24 * 365) + '; path=/'
        }
      },
      readCookie (name) {
      ...
      replaceMedia ({ id }) { this.$refs.uploader.replaceMedia(id) },
      open () { this.$refs.modal.open() },
      close () { this.$refs.modal.hide() },
      opened () {
        // restore last folder before first grid load
        const saved = this.getSavedFolderPath()
      ...
      clearFilters () {
        ...
        this.$nextTick(function () { self.submitFilter() })
      },
      ...
      loadFolderTree () {
        api.getFolders(
          this.endpoint,
          { type: this.type },
          (resp) => { this.folderTree = resp.data.tree || { name: '', children: [] } },
          (error) => {
      ...
      selectFolderPath (pathArray) {
        ...
        this.saveLastFolder()          // persist selection
        this.submitFilter()
      },
      ...
      clearSelectedMedias () { this.selectedMedias.splice(0) },
      deleteSelectedMedias (mediasIds) {
        ...
        mediasIds.forEach(() => { this.$store.commit(MEDIA_LIBRARY.DECREMENT_MEDIA_TYPE_TOTAL, this.type) })
        this.mediaItems = this.mediaItems.filter(media => !this.selectedMedias.includes(media) || keepSelectedMedias.includes(media))
        this.selectedMedias = keepSelectedMedias
        if (this.mediaItems.length <= 40) { this.reloadGrid() }
      },
      clearMediaItems () { this.mediaItems.splice(0) },
      reloadGrid () {
        this.loading = true
        ...
        api.get(
          ...
          (error) => {
            this.$store.commit(NOTIFICATION.SET_NOTIF, { message: error.data.message, variant: 'error' })
          }
        )
      },
      reloadTags (tags = []) { this.tags = tags },
      submitFilter () {
        const self = this
...
<style lang="scss">
  ...
  .medialibrary__filter-item.checkbox { margin-top: 8px; margin-right: 45px !important; }
  ...
  .medialibrary { display: block; width: 100%; min-height: 100%; padding: 0; position: relative; }
  .medialibrary__header { background: $color__border--light; border-bottom: 1px solid $color__border; padding: 0 20px; }

  .medialibrary__frame { position: absolute; top: 0; left: 0; right: 0; bottom: 0; display: flex; flex-flow: column nowrap; }
  ...
  .medialibrary__foldertree {
    ...
    width: 220px; max-height: calc(100vh - 220px); overflow: auto;
    border-right: 1px solid #eee; padding: 8px 0;
  ...
  .medialibrary__list {
    margin: 0; position: absolute; top: 0; left: 220px; right: map-get($width_sidebar, default); bottom: 0;
  ...
  .folder-node__row {
    ...
    line-height: 1.8; padding: 2px 6px;
  }
  .folder-node__toggle,
  .folder-node__name,
  .folder-node__create {
  ...
  .folder-node__name.is-active { font-weight: 600; text-decoration: underline; }
