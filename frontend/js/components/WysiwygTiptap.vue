<template>
  ...
  >
        ...
        <div class="wysiwyg__editor" ref="editor">
          <div class="wysiwyg__menubar">
            ...
            <template v-if="this.toolbar.wrappers">
              ...
              <template
                v-for="wrapper in this.toolbar.wrappers"
                :key="wrapper.id"
              >
                <wysiwyg-menu-bar-btn
                  :icon-url="wrapper.icon"
    ...
    />
    <a17-modal
      :title="$trans('wysiwyg.link_window.title', 'Edit link')"
      ref="link-modal"
    ...
    >
      <template v-if="linkWindow">
        ...
        <a17-textfield
          ...
          :placeholder="
            $trans('wysiwyg.link_window.link_placeholder', 'https://...')
          "
        />
        <div>
          <a17-button
            class="link-browser-link"
            variant="aslink-grey"
            v-if="browserEndpoints"
            @click="browserIsOpen = true"
          >
            {{
              $trans(
                'wysiwyg.link_window.internal_browser_link',
        ...
        <a17-inputframe name="link-options">
          <a17-checkbox
            ...
            :label="
              $trans(
                'wysiwyg.link_window.open_in_new_window',
                'Open in a new tab'
              )
            "
          />
          <div
            class="classList"
            v-if="
              linkWindow && linkWindow.classList && linkWindow.classList.length
            "
          >
            <a17-checkbox
              v-for="(item, index) in linkWindow.classList"
  ...
  export default {
    ...
    methods: {
        ...
        this.linkWindow = {
          ...
          classList: this.classList
            ? this.classList.map(item => {
                item.selected = (markAttributes.class ?? '').includes(
                  item.value
                )
                return item
              })
      ...
      setLinkFromBrowser(item) {
        this.linkWindow.href =
          '#twillInternalLink::' + item[0].endpointType + '#' + item[0].id
      },
      saveLink() {
        ...
        if (this.linkWindow.newLink) {
          this.editor.commands.setLink({
            href: this.linkWindow.href,
            target: this.linkWindow.target,
            class: this.linkWindow.classList
              .filter(item => item.selected)
              .map(item => item.value)
              .join(' ')
          })
        } else {
          this.editor.commands.updateAttributes('link', {
            href: this.linkWindow.href,
            target: this.linkWindow.target,
            class: this.linkWindow.classList
              .filter(item => item.selected)
