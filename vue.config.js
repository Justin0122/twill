...
process.env.VUE_APP_CUSTOM_COMPONENTS_PATH =
  process.env.VUE_APP_CUSTOM_COMPONENTS_PATH ?? '@/components/customs/'
...
const useHttps = process.env.TWILL_DEV_MODE_SSH
  ? process.env.TWILL_DEV_MODE_SSH === 'true'
  : false
...
if (
  fs.existsSync(`${srcDirectory}/icons-custom`) &&
  fs.readdirSync(`${srcDirectory}/icons-custom`).length !== 0
) {
